...
/*   Updated: 2019/07/25 15:56:28 by ajulanov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				draw_right(t_map *map);
void				draw_down(t_map *map);
void				draw(t_map *map);
...
{
	map->y1 = 0;
		while (map->y1 < map->height)
	{
		map->x1 = 0;
		while (map->x1 < map->width)
		{
			
			map->x2 = map->x1 + map->gap;
			map->y2 = map->y1;
			if (map->x1 + 1 < map->width)
				draw_line(map);
			map->y2 = map->y1 + map->gap;
			if (map->y1 + 1 < map->height)
				draw_line(map);
			map->x1 += map->gap;
			map->x2 = map->x1;
		}
...
// 	map->h = 0;
// 	map->w = 0;
// 	while (map->h  < map->height)
// 	{
// 		map->w = 0;
// 		while (map->w < map->width)
// 		{
// 			printf("1: %d  %d\n", map->h, map->w);
// 			map->x1 = map->x_grid[map->h][map->w];
// 			map->y1 = map->y_grid[map->h][map->w];
// 			printf("2 :%d  %d\n", map->h, map->w);
// 		if (map->h + 1 < map->height)
// 		{
// 			map->x2 = map->x_grid[map->h][map->w + 1];
// 			map->y2 = map->x_grid[map->h][map->w + 1];
// 		}
// 		draw_line(map);
// 		map->w++;
// 		}
...
// 	map->h = 0;
// 	map->w = 0;
// 	// map = (int **)ft_memalloc(sizeof(int *) *map->h);
// 	while (map->h < map->height)
// 	{
// 		map->w = 0;
// 				while (map->w + 1 < map->width)
// 		{
// 			printf("1: %d  %d\n", map->h, map->w);
// 			map->x1 = map->x_grid[map->h][map->w];
// 			map->y1 = map->y_grid[map->h][map->w];
...
// 		if (map->h < map->height)
// 		{
// 			map->x2 = map->x_grid[map->h + 1][map->w];
// 			map->y2 = map->x_grid[map->h + 1][map->w];
// 		}
// 		draw_line(map);
// 		map->w++;
// 		}
